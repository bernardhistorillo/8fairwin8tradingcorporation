...
class TerminalController extends Controller
{
    public function index(Request $request, $view = null) {
        ...
        if(empty($request->type)) {
            ...
            $type = 1;
            if($terminalUser) {
                if($terminalUser["package_id"] == 1 || $terminalUser["package_id"] == 3 || $terminalUser["package_id"] == 7) {
                    return redirect()->route('terminal.index');
                }
            }
        } else {
        ...
        $showProductsTab = (!$terminalUser && (Auth::user()->package_id != 1 || Auth::user()->package_id != 3 || Auth::user()->package_id != 7)) || ($terminalUser && ($terminalUser["package_id"] != 1 || $terminalUser["package_id"] != 3 || $terminalUser["package_id"] != 7));
