...
<div class="row">
    <div class="col-lg-6 col-xl-8">
        <nav id="products-tab-container" class="nav nav-pills nav-justified mb-4 {{ (!$showProductsTab) ? 'd-none' : '' }}" data-hidden="{{ (!$showProductsTab) ? '1' : '0' }}">
            <a class="nav-link {{ ($type == 2 || $type == 0) ? 'active' : '' }} products-tab" data-type="2" style="cursor:pointer">Products</a>
            <a class="nav-link {{ ($type == 1) ? 'active' : '' }} products-tab" data-type="1" data-package-id="{{ ($terminalUser) ? $terminalUser["package_id"] : Auth::user()->package_id }}" style="cursor:pointer">Packages</a>
        </nav>

        <div class="row align-items-stretch px-1">
            @foreach($items as $item)
            <div class="col-md-6 col-lg-12 col-xl-6 px-2 products-section d-none" data-type="{{ $item["type"] }}" data-package-id="{{ $item["package_id"] }}">
                <div class="h-100 pb-4">
                    <div class="card border-radius-0 product-container h-100" data-id="{{ $item["id"] }}"
                        ...
                        @if($terminalUser)
                        <span class="stock d-none">{{ $item->terminalItemStock(Auth::user())['inStock'] }}</span>
                        @endif
